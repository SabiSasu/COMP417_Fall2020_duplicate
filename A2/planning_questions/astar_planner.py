        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                if (ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    ...
                    #g(n) + h(n) where h(n) is the euclidian distance between point and goal
                    Q[ns] = best_dist_found[ns.x, ns.y] + (sqrt((ns.x - dest_state.x)**2 + (ns.y - dest_state.y)**2))
                    parents[ns] = s
...
if __name__ == "__main__":
    ...
    states = [[350, 280], [50, 330], [150, 233]]
    for i in range(len(states)):
        dest_state = State(states[i][0],states[i][1])
        ...
        filename = 'astar_result' + str(i) + '_SabinaSasu.png'
