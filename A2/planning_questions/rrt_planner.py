...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        distance = sqrt((s_nearest.x - s_rand.x)**2 + (s_nearest.y - s_rand.y)**2)
        #s_rand is within the radius
        x = s_rand.x
        y = s_rand.y
        #s_rand is outside the radius, x and y are distance r towards the s_rand direction
        if distance > max_radius:
            y_space = -(s_rand.y - s_nearest.y)
            x_space = s_rand.x - s_nearest.x
            angle = 0
            if x_space != 0:
                angle = math.atan(abs(y_space) / abs(x_space))
                x = (int)(s_nearest.x + (math.cos(angle) * max_radius))
                y = (int)(s_nearest.y - (math.sin(angle) * max_radius))
                if y_space > 0 and x_space < 0:
                    x = (int)(s_nearest.x - (math.cos(angle) * max_radius))
                    y = (int)(s_nearest.y - (math.sin(angle) * max_radius))
                elif y_space < 0 and x_space < 0:
                    x = (int)(s_nearest.x - (math.cos(angle) * max_radius))
                    y = (int)(s_nearest.y + (math.sin(angle) * max_radius))
                elif y_space < 0 and x_space > 0:
                    x = (int)(s_nearest.x + (math.cos(angle) * max_radius))
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            s_new = State(0, 0, s_from)
            s_new.x = (int)(float(i) / max_checks * (s_to.x - s_from.x)) + s_from.x
            s_new.y = (int)(float(i) / max_checks * (s_to.y - s_from.y)) + s_from.y
...
if __name__ == "__main__":
    ...
    states = [[250, 10], [150, 233], [10, 10]]
    for i in range(len(states)):
        dest_state = State(states[i][0], states[i][1], None)
        ...
        plan = rrt.plan(start_state,
                        ...
                        dest_reached_radius,
                        live_view=True)
        filename = 'rrt_result' + str(i) + '_SabinaSasu.png'
        print('RRT planning complete. Saving image.')
        draw_plan(world, plan, bgr=(0,0,255), thickness=2, show_live=False,filename=filename)
